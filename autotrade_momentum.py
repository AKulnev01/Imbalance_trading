...
def _summarize_fills(fills, side, entry_px_adj):
    tp, sl = [], []
    rem = 1.0
    rows = []
    for i, f in enumerate(fills):
        reason = str(f.get("reason",""))
        frac   = float(f.get("frac", 0.0))
        price  = float(f.get("price", float('nan')))
        ts     = pd.to_datetime(f.get("ts"), utc=True, errors="coerce")
        leg_side = "TP" if reason.startswith("tp") else ("SL" if reason.startswith("sl") else reason)
        leg_move_pct = ((price - entry_px_adj)/entry_px_adj*100.0) if side=="BUY" else ((entry_px_adj - price)/entry_px_adj*100.0)
        rows.append({
            "leg_idx": i+1, "leg_side": leg_side, "leg_reason": reason,
            "leg_frac": frac, "leg_price": price, "leg_ts": ts,
            "leg_pnl_pct": leg_move_pct, "remaining_after": max(0.0, rem - frac),
        })
        if reason.startswith("tp"): tp.append((ts, price, frac))
        if reason.startswith("sl"): sl.append((ts, price, frac))
        rem = max(0.0, rem - frac)

    def _pack(items):
        if not items:
            return (0, 0.0, "", "")
        items_sorted = sorted(items, key=lambda x: (x[0] if pd.notna(x[0]) else pd.Timestamp.max))
        cnt  = len(items_sorted)
        fsum = round(sum(fr for _,_,fr in items_sorted), 6)
        prices = ";".join([f"{p:.8g}" for _,p,_ in items_sorted])
        times  = ";".join([pd.to_datetime(t).strftime("%Y-%m-%d %H:%M:%S%z") if pd.notna(t) else "" for t,_,_ in items_sorted])
        return (cnt, fsum, prices, times)

    tp_cnt, tp_fsum, tp_prices, tp_times = _pack(tp)
    sl_cnt, sl_fsum, sl_prices, sl_times = _pack(sl)
    return {
        "tp_legs_count": tp_cnt, "tp_legs_frac_sum": tp_fsum,
        "tp_legs_prices": tp_prices, "tp_legs_times": tp_times,
        "sl_legs_count": sl_cnt, "sl_legs_frac_sum": sl_fsum,
        "sl_legs_prices": sl_prices, "sl_legs_times": sl_times,
    }, rows
...
def _evaluate_one_variant(row: pd.Series, *, variant_name: str, symbol: str, side: str, entry_at: pd.Timestamp, entry_px_ref: float, df4h: pd.DataFrame, as_of: pd.Timestamp, max_days: int):
    """Возвращает (out_dict, fill_rows:list)"""
    if pd.isna(entry_at) or not isinstance(entry_px_ref, (int, float)) or math.isnan(entry_px_ref):
        out = row.to_dict(); out.update({
            ...
            "slot_free_time": pd.NaT,
            "fills_json": "[]",
            "tp_legs_count": 0, "tp_legs_frac_sum": 0.0, "tp_legs_prices": "", "tp_legs_times": "",
            "sl_legs_count": 0, "sl_legs_frac_sum": 0.0, "sl_legs_prices": "", "sl_legs_times": "",
        }); return out, []
    ...
    if ENABLE_SCALE_OUT:
        ...
        fills = sim.get("fills", [])
        summary, fill_rows = _summarize_fills(fills, side, float(entry_px_adj))
...
def _simulate_limit_backfill(row_out_dict, row_sig: pd.Series, df4h: pd.DataFrame, as_of: pd.Timestamp):
    """Возвращает (out_dict_or_None, fill_rows:list)"""
    ...
    # лимит на entry close 4h
    _, limit_px_ref = _entry_from_4h_close(df4h, t0)
...
def evaluate_momentum(signals_path: str, result_path: str, lookback_days: int = 360, interval: str = "4h", max_days: int = None, only_filled: bool = False, dedup: bool = False, initial_capital: float = None, capital_aware: bool = True):
    ...
    results = []
    fills_accum = []  # сюда собираем все ноги
    for _, row in df_sig.iterrows():
        ...
        out_std, fill_rows = _evaluate_one_variant(
        ...
        for r in fill_rows:
            r.update({"symbol": symbol, "variant": "MOMENTUM", "imb_time": t0, "type": side})
            fills_accum.append(r)
        ...
        if ENABLE_EARLY_CHECK and EARLY_CHECK_MIN_BEFORE_CLOSE and EARLY_CHECK_MIN_BEFORE_CLOSE > 0:
            ...
            if pd.notna(entry_at_e) and isinstance(entry_px_ref_e, (int, float)) and not math.isnan(entry_px_ref_e):
                ...
                out_e, fill_rows_e = _evaluate_one_variant(
                ...
                for r in fill_rows_e:
                    r.update({"symbol": symbol, "variant": variant_name, "imb_time": t0, "type": side})
                    fills_accum.append(r)
            else:
                results.append({
                    **row.to_dict(),
    ...
    if LIMIT_BACKFILL_ENABLE and ENFORCE_ONE_AT_A_TIME and MAX_CONCURRENT and MAX_CONCURRENT > 0:
        ...
        if skipped_idx:
            sig_indexed = pd.DataFrame(results).reset_index(drop=True)  # используем исходные сигналы по индексу
            for i in skipped_idx:
                ...
                if i in df_res.index and "_backfill_start" in df_res.columns:
                    try:
                        row_out_dict["_backfill_start"] = df_res.loc[i, "_backfill_start"]
                    except Exception:
                        pass
                backfill_out, bf_rows = _simulate_limit_backfill(row_out_dict, sig_row, df4h, as_of)
                if isinstance(backfill_out, dict):
                    backfill_rows.append(backfill_out)
                    for r in bf_rows:
                        r.update({"symbol": symbol, "variant": "MOMENTUM_LMT_BACKFILL", "imb_time": sig_row.get("imb_time"), "type": sig_row.get("type")})
                        fills_accum.append(r)
    ...
    # Лист с ногами
    df_fills = pd.DataFrame(fills_accum) if fills_accum else pd.DataFrame(columns=[
        "symbol","variant","imb_time","type","leg_idx","leg_side","leg_reason","leg_frac","leg_price","leg_ts","leg_pnl_pct","remaining_after"
    ])
    if not df_fills.empty:
        extra["fills_legs"] = df_fills

    # Excel-friendly times (naive)
    ...
    if isinstance(eq_sheet, pd.DataFrame):
        ...
        for c in eq_time_cols:
            try:
                ...
                pass
    if not df_fills.empty:
        for c in ["imb_time","leg_ts"]:
            try:
                s = pd.to_datetime(df_fills[c], errors="coerce", utc=True)
                df_fills[c] = s.dt.tz_convert(None)
            except Exception:
                pass
